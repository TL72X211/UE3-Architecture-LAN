...
- Commutateur : Switch, pas d'accès à internet (réservé aux Routeurs), relis plusieurs équipement, couche 2, possède une table d'adresse MAC.
- Domaine de diffusion : Correspond à une zone d'un réseau, dans laquelle une trame se **diffuse** librement, délimitée par les équipements de couche 3. Tout ce qui est dans un domaine reçoit les diffusions
- Réseau local : Réseau LAN, infrastructure réseau couvrant une zone peu étendue. Souvent administré par une seule entreprise ou une seule personne
...
- Plage IP : 192.168.25.1 -- 192.168.25.35 (par ex) (aussi Pool d'adresses)
...
- Domaine de diffusion, domaine de collision
	Deux périphériques sont dans le même domaine de collision lorsque l'envoi de données simultané sur la même voix provoque une corruption de ces données.(Existe uniquement )  
...
- Architecture 3 couches
	...
	Résumé : **INTERNET** ou **Fibre Noire**
	...
	Résumé : **Switchs** de l'entreprise
...
- Configurer les switches de façon sécurisée (VTP, ports)  
	...
	Protocle de couche 2, permettant de diffuser des annonces de création, suppression ou modification de VLAN. Utilie une adresse MAC bien particulière "01-00-0C-CC-CC-CC"  
	3 types de switch VTP :
	 * VTP Serveur : Switch sur lequel on créé les VLAN et qui va les diffuser aux autres
	 * VTP Client : Switch qui transmet à ses voisins et qui va se synchroniser aux autres switchs VTP du même "groupe"
	 * VTP Transparent : Switch relayant les informations VTP mais qui ne se synchronise pas
	A chaque envoi de message VTP, la variable *Revision Number* qui est stockée sur les switchs et a chaque passage du message VTP (par défaut toutes les 5min) les switchs vérifient leur version, si le *Revision Number* local est inférieur, il s'incrémente et le switch se synchronise.
	Pour le configurer :  
	**Switch>enable  
	Switch#configure terminal  
	Switch(config)#vtp domain TEST**  
	*On change le domaine VTP de NULL' à 'TEST'*  
	On passe ensuite le switch serveur en mode serveur  
	**Switch(config)#vtp mode server**  
	On configure ensuite un mot de passe
	**Switch(config)#vtp password cisco123**  
	Activation de la version sur chaque switch  
	**Switch(config)#vtp version 2**

	Pour configurer un switch en client :
	* On bascule en mode Transparent pour mettre son RN à zéro avant de le rebasculer en mode Client  
	...
	* Pour vérifier la configuration on utilise la commande
...
- VLAN
	...
	Des plages de VLAN existent et certaines sont réservées : (TAG, en-tête, ajouté dans la trame)
